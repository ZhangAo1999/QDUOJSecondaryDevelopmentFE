  ...
  <Row type="flex" justify="space-around">
    <Col :span="20" id="status">
      <Alert :type="status.type" showIcon>
        ...
        <div v-if="submission.can_unshare" id="share-btn" style="margin-top:20px">
          <Button :class="['ivu-btn-theme-' + theme, 'ivu-btn-ghost']" v-if="submission.shared"
                  type="warning" size="large" @click="shareSubmission(false)">
            {{$t('m.UnShare')}}
          </Button>
          <Button :class="['ivu-btn-theme-' + theme, 'ivu-btn-ghost']" v-else
    ...
    <Col v-if="submission.info && !isCE" :span="20">
      <Table :class="'ivu-table-' + theme" :loading="loading" :disabled-hover="true" :columns="columns" :data="submission.info.data"></Table>
    </Col>
