      ...
      <Panel shadow>
        ...
        <div slot="extra">
          <ul class="filter">
            ...
            <li>
              <Input v-model="problemID" :placeholder="$t('m.Search_Problem')" @on-enter="handleQueryChange"/>
            </li>
            <li>
            ...
            <li>
              <Button :class="['ivu-btn-theme-' + theme, 'ivu-btn-ghost']" type="info" icon="refresh" @click="getSubmissions">{{$t('m.Refresh')}}</Button>
            </li>
          </ul>
        </div>
        <Table :class="'ivu-table-' + theme" :disabled-hover="true" :columns="columns" :data="submissions" :loading="loadingTable" 
            @on-sort-change="changeSortBy"
            @on-row-dblclick="copyCode"></Table>
...
<script>
  ...
  export default {
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('div', [
                h('button', {
                  class: 'ivu-tag-' + JUDGE_STATUS[params.row.result].color,
                  style: {
    ...
    methods: {
      changeSortBy (res) {
        this.sortBy = res.key + '_' + res.order
        this.handleQueryChange()
      },
      copyCode (params) {
        if (!params.show_link) { return }
        const id = params.id
        api.getSubmission(id).then(codeRes => {
          let code = codeRes.data.data.code
          this.$copyText(code).then(res => {
